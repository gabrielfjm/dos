document.addEventListener("DOMContentLoaded", function() {
    ...
    // Define a data atual como padrão no campo "Data de Término"
    document.getElementById('data_termino_input').value = hoje;
    ...
    function carregarOrigens() {
        $.ajax({
            url: 'http://localhost:8080/origens',
            method: 'GET',
    ...
    function formatarNumeroChamado(event) {
        ...
        if (chamado.length === 4) {
            ...
            //chamado = chamado.replace(/\D/g, '').replace(/(\/.*)/, '');
    ...
    document.getElementById('form_cadastro_computador').addEventListener('submit', function(event) {
        const patrimonio = document.getElementById('patrimonio_input');
        ...
        validarCampoAntesSubmit(patrimonio, 'erroPatrimonio');
    ...
    validarCampo(document.getElementById('patrimonio_input'), 'erroPatrimonio');
